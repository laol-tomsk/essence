...
class NeoByse(object):
	...
	def searth_node_by_name(self,name_node,name_database):
		if (self.__checkbaseexistence__(name_database,self.get_bases())):
	...
	#Функция для поиска ноды в базеданных
	def searth_node_by_guid(self,guid_node,name_database):
		if (self.__checkbaseexistence__(name_database,self.get_bases())):
			tx = self.__congraph__(name_database)
...
#	def return_all_parents_for_the_node(self,name_database,node_name = None, node_guid = None):
#		if (self.__checkbaseexistence__(name_database,self.get_bases())):
#			tx = self.__congraph__(name_database)
#			mas_parents = []
#			for rel_parent in (tx.match((None,self.searth_node_by_guid(node_guid,name_database))),tx.match((None,self.searth_node_by_name(node_name,name_database))))[node_name != None]:
...
#		pass
	...
	def return_all_parents_for_the_node(self,tx,name_database,node_name = None, node_guid = None):
		start_time = time.time()
		#tx = self.__congraph__(name_database)
		mas_parents = []
		for rel_parent in (tx.match((None,self.searth_node_by_guid(tx,node_guid,name_database))),tx.match((None,self.searth_node_by_name(tx,node_name,name_database))))[node_name != None]:
			mas_parents.append(rel_parent.start_node)
		print("--- %s seconds ---" % (time.time() - start_time))
		return mas_parents

	def return_all_nodes_by_labels(self,name_database,labels):
		if (self.__checkbaseexistence__(name_database,self.get_bases())):
			tx = self.__congraph__(name_database)
...
class parser_json(object):
	def __init__(self,file):
		...
		self.Вegree_of_influence = {'2': 'S', '1': 'M', '0': 'W'}
		self.dict_state_id = {}
