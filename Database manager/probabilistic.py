...
class CalculatingTheProbability(object):
	...
	def calculete(self,iteration,node_name):
		...
		if node_name in self.mas_dictionary_nodes[iteration]:
			if self.mas_dictionary_nodes[iteration][node_name] == -1:
				...
				if iteration != 0:
					X = 0
					for vector in self.__get_weight_value__(node_name,self.data_dict[node_name][iteration]):
						Y = vector[1]
						#print(node_name +" Y = "+ str(Y))
						for parent3 in parents:
	...
	def a(self):
		...
		final_dict = {key:self.data_dict[key] for key in self.data_dict.keys()}
		for key in self.data_dict.keys():
			if not(key in self.mas_dictionary_nodes[0]) and re.fullmatch(r'\D+\d{1}',key):
				final_dict.update({key:self.data_dict[key]})
